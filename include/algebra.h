 ...
 * \brief Finds the eigen vector of a square matrix NxN
 * \param mat Target matrix
 * \param eig_val A eigen value of mat
 * \return A eigen vector (Nx1) corresponding to the given eigen value
 */
struct matrix *algebra_mat_eigen_vec1(struct matrix *mat, cnum eig_val);

/**
 * \brief Calculates the euclidean norm of a vector Nx1
 * \param mat Target matrix
 * \return The norm of the vector, or -1 if it fails
 */
cnum algebra_vec_norm(struct matrix *mat);

/**
 * \brief Normalizes a vector (Nx1) using the euclidean norm
 * \param mat Target matrix
