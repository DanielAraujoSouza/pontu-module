...
struct cloud *voxelgrid_sampling_centroid(struct cloud *src, real leafsize)
{
    ...
    uint len_x = (uint)ceil((max_x - min_x) / leafsize);
    uint len_y = (uint)ceil((max_y - min_y) / leafsize);
    uint len_z = (uint)ceil((max_z - min_z) / leafsize);
    ...
    {
        uint pos_x = (uint)ceil((s->point->x - min_x) / leafsize) - 1;
        uint pos_y = (uint)ceil((s->point->y - min_y) / leafsize) - 1;
        uint pos_z = (uint)ceil((s->point->z - min_z) / leafsize) - 1;
