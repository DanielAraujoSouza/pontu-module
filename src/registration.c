...
struct matrix *registration_align(struct cloud *source, struct cloud *target)
{
  ...
  struct matrix *ns = algebra_mat_vs_scalar(s, 1.0 / ((double)source->numpts));
  matrix_free(&s);

  struct matrix *st = algebra_mat_transpose(ns);
  struct matrix *a = algebra_mat_sub(ns, st);
  matrix_free(&st);
  struct matrix *q = matrix_new(4, 4);

  real tr_s = creal(algebra_mat_trace(ns));
  ...
  matrix_set(q, 1, 1, matrix_get(ns, 0, 0) * 2.0 - tr_s);
  matrix_set(q, 2, 2, matrix_get(ns, 1, 1) * 2.0 - tr_s);
  matrix_set(q, 3, 3, matrix_get(ns, 2, 2) * 2.0 - tr_s);

  matrix_set(q, 1, 2, matrix_get(ns, 0, 1) + matrix_get(ns, 1, 0));
  matrix_set(q, 1, 3, matrix_get(ns, 0, 2) + matrix_get(ns, 2, 0));
  matrix_set(q, 2, 3, matrix_get(ns, 1, 2) + matrix_get(ns, 2, 1));
  ...
  struct matrix *rs = algebra_mat_prod(r, source_aux);
  struct matrix *t = algebra_mat_sub(target_aux, rs);
  matrix_free(&rs);
                                ...
                                closest_points_func closest)
{
  ...
  *aligned = cloud_copy(source);
  ...
  do
  {
    ...
    cur_rmse = cloud_rmse_sorted(nsrc, ntgt);
