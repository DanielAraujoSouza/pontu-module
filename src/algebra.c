...
struct matrix *algebra_quadratic_roots(real a, real b, real c)
{
  ...
  matrix_set(roots, 0, 0, (-b + csqrt(delta)) / (2.0 * a));
  matrix_set(roots, 1, 0, delta == 0.0 ? matrix_get(roots, 0, 0) : (-b - csqrt(delta)) / (2.0 * a));
...
struct matrix *algebra_cubic_roots(real a, real b, real c, real d)
{
  ...
  if (delta == 0.0)
  {
    ...
    else
    {
      ...
      matrix_set(roots, 1, 0, matrix_get(roots, 0, 0));
      matrix_set(roots, 2, 0, (4.0 * a * b * c - 9.0 * a * a * d - b * b * b) / (a * delta_0));
    }
  }
  else
  {
...
struct matrix *algebra_gauss_elim(struct matrix *mat)
{
  ...
  for (int l = (int)(eq_sys->rows - 1); l >= 0; l--)
  {
    matrix_set(solution, (uint)l, 0, matrix_get(eq_sys, (uint)l, eq_sys->cols - 1));
    for (uint j = (uint)l + 1; j < (eq_sys->cols - 1); j++)
      matrix_set(solution, (uint)l, 0,
                 matrix_get(solution, (uint)l, 0) -
                     matrix_get(eq_sys, (uint)l, j) *
                         matrix_get(solution, j, 0));
    matrix_set(solution, (uint)l, 0,
               matrix_get(solution, (uint)l, 0) / matrix_get(eq_sys, (uint)l, (uint)l));
  }
...
struct matrix *algebra_mat_eigen_val(struct matrix *mat)
{
  ...
  else if (mat->rows == 4)
  {
    ...
    real coefc = a * (k + f + p) - (-p * (k + f) - f * k + n * h + o * l + j * g) -
                 b * e +
                 c * (-i) -
                 d * m;
                                     ...
                                     struct matrix *eig_val)
{
  ...
  matrix_set(used_eig_val, 0, 1, -1.0);
...
struct matrix *algebra_mat_eigen_vec1(struct matrix *mat, cnum eig_val)
{
  struct matrix *mat_cpy;

  struct matrix *eig_vec = matrix_new(mat->rows, 1);
  ...
  for (uint k = 0; k < mat_cpy->rows; k++)
    matrix_set(mat_cpy, k, k, matrix_get(mat_cpy, k, k) - eig_val);
  ...
  for (uint j = 0; j < eig_vec->rows; j++)
    matrix_set(eig_vec, j, 0, matrix_get(temp_vec, j, 0));
...
cnum algebra_vec_norm(struct matrix *mat)
{
  if (mat == NULL)
    return -1;
  ...
  cnum norm = 0;
  for (uint i = 0; i < mat->rows; i++)
    norm += matrix_get(mat, i, 0) * matrix_get(mat, i, 0);
...
{
  cnum norm = algebra_vec_norm(mat);

  if (norm == -1)
    return NULL;

  return algebra_mat_vs_scalar(mat, 1.0 / norm);
}